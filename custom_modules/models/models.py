...
class AccountMoveReversalInherit(models.TransientModel):
    ...
    def reverse_moves(self):
        ...
        for line in range(len(self.new_move_ids.line_ids)):
            ...
            _logger.info(record)
            _logger.info(record.account_origin_id.code)
            ...
            _logger.info(self.env['account.account'].search([('code','=',record.account_origin_id.code if len(record) > 0 else self.new_move_ids.line_ids[line].account_id.code)]))
            self.new_move_ids.line_ids[line].account_id = self.env['account.account'].search([('code','=',record.account_destination_id.code if len(record) > 0 else self.new_move_ids.line_ids[line].account_id.code)])
            #_logger.info("******", self.env('account.account').search([('code','=',record.code if record.id else self.new_move_ids.line_ids.account_id.code)]).id)
